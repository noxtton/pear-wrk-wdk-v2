...
const encoding18 = {
  preencode (state, m) {
    ...
    if (version >= 2 && m.address) c.string.preencode(state, m.address)
  },
  encode (state, m) {
    const flags =
      (m.eoaAddress ? 1 : 0) |
      ((version >= 2 && m.address) ? 2 : 0)

    c.uint.encode(state, flags)

    if (m.eoaAddress) c.string.encode(state, m.eoaAddress)
    if (version >= 2 && m.address) c.string.encode(state, m.address)
  },
  decode (state) {
    const flags = c.uint.decode(state)
    ...
    return {
      ...
      address: (version >= 2 && (flags & 2) !== 0) ? c.string.decode(state) : null
    }
  }
}
...
const encoding48 = {
  preencode (state, m) {
    ...
    if (version >= 3 && m.enableDebugLogs) c.uint.preencode(state, m.enableDebugLogs)
    c.string.preencode(state, m.allowedNetworks)
    c.string.preencode(state, m.config)
  },
  encode (state, m) {
    const flags = (version >= 3 && m.enableDebugLogs) ? 1 : 0

    c.uint.encode(state, flags)

    if (version >= 3 && m.enableDebugLogs) c.uint.encode(state, m.enableDebugLogs)
  ...
  decode (state) {
    ...
    return {
      enableDebugLogs: (version >= 3 && (flags & 1) !== 0) ? c.uint.decode(state) : 0,
...
const encoding49 = {
  preencode (state, m) {
    ...
    if (version >= 3 && m.status) c.string.preencode(state, m.status)
  },
  encode (state, m) {
    const flags = (version >= 3 && m.status) ? 1 : 0

    c.uint.encode(state, flags)

    if (version >= 3 && m.status) c.string.encode(state, m.status)
  },
  decode (state) {
    const flags = c.uint.decode(state)

    return {
      status: (version >= 3 && (flags & 1) !== 0) ? c.string.decode(state) : null
    }
  }
}
...
function getEncoding (name) {
  switch (name) {
    ...
    case '@wdk-core/wdkReadOnlyInit-request': return encoding48
    case '@wdk-core/wdkReadOnlyInit-response': return encoding49
