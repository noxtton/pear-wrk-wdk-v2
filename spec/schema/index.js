...
const encoding22 = {
  preencode (state, m) {
    ...
    if (version >= 2 && m.transferMaxFee) c.string.preencode(state, m.transferMaxFee)
  },
  encode (state, m) {
    const flags =
      (m.paymasterToken ? 1 : 0) |
      ((version >= 2 && m.transferMaxFee) ? 2 : 0)

    c.uint.encode(state, flags)

    if (m.paymasterToken) encoding22_0.encode(state, m.paymasterToken)
    if (version >= 2 && m.transferMaxFee) c.string.encode(state, m.transferMaxFee)
  },
  decode (state) {
    const flags = c.uint.decode(state)
    ...
    return {
      ...
      transferMaxFee: (version >= 2 && (flags & 2) !== 0) ? c.string.decode(state) : null
    }
  }
}
