...
// @wdk-core/getBatchAbstractedAddresses-request
const encoding48 = {
  preencode (state, m) {
    c.string.preencode(state, m.network)
    c.string.preencode(state, m.accountIndices)
  },
  encode (state, m) {
    c.string.encode(state, m.network)
    c.string.encode(state, m.accountIndices)
  },
  decode (state) {
    const r0 = c.string.decode(state)
    const r1 = c.string.decode(state)
...
// @wdk-core/getBatchAbstractedAddresses-response
const encoding49 = {
  preencode (state, m) {
    state.end++ // max flag is 1 so always one byte

    if (version >= 2 && m.addresses) c.string.preencode(state, m.addresses)
  },
  encode (state, m) {
    const flags = (version >= 2 && m.addresses) ? 1 : 0

    c.uint.encode(state, flags)

    if (version >= 2 && m.addresses) c.string.encode(state, m.addresses)
  },
  decode (state) {
    const flags = c.uint.decode(state)

    return {
      addresses: (version >= 2 && (flags & 1) !== 0) ? c.string.decode(state) : null
    }
  }
}
...
function getEncoding (name) {
  switch (name) {
    ...
    case '@wdk-core/getBatchAbstractedAddresses-request': return encoding48
    case '@wdk-core/getBatchAbstractedAddresses-response': return encoding49
